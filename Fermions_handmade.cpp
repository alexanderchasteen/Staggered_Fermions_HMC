...
double compute_Tr_P2(const std::vector<double>& P_field) {
    ...
    for (int i = 0; i < conjugate_field_array_size; i++) {
        computation += P_field[i] * P_field[i];
    }
    return computation;
}
                ...
                for (int k = 0; k < Spatial_Size; k++) {
                    for (int l = 0; l < temporal_size; l++) {
                        for (int d = 0; d < 4; d++) {
                            ...
                            P_pre = epsilon * get_SU3_at_link(P_field, link_index_array);
    ...
    double beta) 
{
    ...
    double final_phi_field_action = compute_phi_dagger_DDdagger_inverse_phi(phi_field, gauge_field_final);
